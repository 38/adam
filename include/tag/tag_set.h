 ...
 **/
typedef const char* (*tag_set_reasoncode_to_string)(uint32_t tagid, int why);
 ...
 **/
int tag_set_change_resolution(tag_set_t* set, uint32_t tagid, uint32_t value);

/**
 * @brief register a checker for specify tag
 * @param tagid the tagid
 * @param callback the callback fuction
 * @return < 0 if error happends
 **/
int tag_set_register_checker(uint32_t tagid, tag_set_checker_callback_t checker);
