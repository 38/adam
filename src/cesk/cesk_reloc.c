...
{
	hashval_t h = _cesk_store_address_hashcode(fromaddr) % CESK_RELOC_TABLE_SIZE;
	...
	cesk_reloc_table_t* newnode = (cesk_reloc_table_node_t*)malloc(sizeof(cesk_reloc_table_node_t));
	if(NULL == newnode)
	{
...
uint32_t cesk_reloc_table_look_for(const cesk_reloc_table_t* table, uint32_t addr)
{
	...
	int h = _cesk_store_address_hashcode(addr);
	cesk_reloc_table_node_t* ptr;
	for(ptr = table->htab[h]; NULL != ptr; ptr = ptr->next)
