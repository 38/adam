...
int main()
{
	...
	/* create a new object */
	uint32_t addr = cesk_frame_store_new_object(frame, reloc, inst, 0, classpath, dif, inv);
	/* check the result */
	...
	uint32_t addr2 = cesk_frame_store_new_object(frame, reloc, inst2, 0, classpath, dif, inv);
	...
	uint32_t addr3 = cesk_frame_store_new_object(frame, reloc, inst, 0, classpath, dif, inv);
	...
	uint32_t addr4 = cesk_frame_store_new_object(frame, reloc, inst3, 0, classpath, dif, inv);
	/* check the hash code*/
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	/* move reg21, addr4 */
	...
	/* create 3 objects */
	uint32_t obj1 = cesk_frame_store_new_object(frame, reloc, inst, 0, classpath, dif, inv);
	uint32_t obj2 = cesk_frame_store_new_object(frame, reloc, inst2, 0, classpath, dif, inv);
	uint32_t obj3 = cesk_frame_store_new_object(frame, reloc, inst3, 0, classpath, dif, inv);
