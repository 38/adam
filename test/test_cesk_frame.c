...
int main()
{
	...
	assert(0 == cesk_frame_register_load(frame, 0 , CESK_STORE_ADDR_NEG, NULL, NULL));
	/* verify the hashcode */
	...
	assert(0 == cesk_frame_register_move(frame , 0, 1, NULL, NULL));
	/* verify the hashcode */
	assert(cesk_frame_compute_hashcode(frame) == cesk_frame_hashcode(frame));
	/* verify the result */
	assert(1 == cesk_set_equal(frame->regs[0], frame->regs[1]));
	/* try to clear the register */
	assert(0 == cesk_frame_register_clear(frame, 0, NULL, NULL));
	/* verify the hashcode */
	assert(cesk_frame_compute_hashcode(frame) == cesk_frame_hashcode(frame));
	/* verify the result */
	...
	/* create a new object */
	uint32_t addr = cesk_frame_store_new_object(frame, reloc, inst, classpath, NULL, NULL);
	/* check the result */
	assert(CESK_STORE_ADDR_NULL != addr);
	/* verify hashcode */
	assert(cesk_frame_compute_hashcode(frame) == cesk_frame_hashcode(frame));
	/* make the object a active object */
	assert(0 == cesk_frame_register_load(frame, 20, addr, NULL, NULL));
	/* check the refcount */
	assert(1 == cesk_store_get_refcnt(frame->store, addr));
	/* test object get for new object */
	assert(0 == cesk_frame_register_load(frame, 31, addr, NULL, NULL));
	assert(0 == cesk_frame_register_load_from_object(frame, 1, 31, superclass, field, NULL, NULL));
	/* the result should be empty set */
	assert(0 == cesk_set_size(frame->regs[0]));
	/* move reg0, $0 */
	assert(0 == cesk_frame_register_load(frame, 0, CESK_STORE_ADDR_ZERO, NULL, NULL));
	/* verify the size of reg0 */
	assert(1 == cesk_set_size(frame->regs[0]));
	/* object-put addr, antlr.CharScanner.literals, reg0 */
	cesk_frame_store_put_field(frame, addr, 0, superclass, field, NULL, NULL);
	...
	/* move reg2, $-1 */
	assert(0 == cesk_frame_register_load(frame, 2, CESK_STORE_ADDR_NEG, NULL, NULL));
	/* verify the size of reg2 */
	assert(1 == cesk_set_size(frame->regs[2]));
	/* verify the hash code */
	assert(cesk_frame_compute_hashcode(frame) == cesk_frame_hashcode(frame));
	/* object-put addr, antlr.CharScanner.literals, reg2 */
	assert(0 == cesk_frame_store_put_field(frame, addr, 2, superclass, field, NULL, NULL));
	/* verify the hash code */
	assert(cesk_frame_compute_hashcode(frame) == cesk_frame_hashcode(frame));
	/* the value should be overrided , verify it */
	assert(0 == cesk_frame_register_load(frame, 31, addr, NULL, NULL));
	assert(0 == cesk_frame_register_load_from_object(frame, 3, 31, superclass, field, NULL, NULL));
	/* so the value should be same as reg2 */
	assert(cesk_set_equal(frame->regs[2], frame->regs[3]) == 1);

	/* test for cascade decref */
	uint32_t addr2 = cesk_frame_store_new_object(frame, reloc, inst2, classpath, NULL, NULL);
	...
	assert(0 == cesk_frame_register_load(frame, 5, addr2, NULL, NULL));
	/* verify hashcode */
	assert(cesk_frame_compute_hashcode(frame) == cesk_frame_hashcode(frame));
	/* object-put addr, antlr.CharScanner.literals, reg5 */
	assert(0 == cesk_frame_store_put_field(frame, addr, 5, superclass, field, NULL, NULL));
	/* verify the hash code */
	assert(cesk_frame_compute_hashcode(frame) == cesk_frame_hashcode(frame));
	/* verify the refcount */
	...
	uint32_t addr3 = cesk_frame_store_new_object(frame, reloc, inst, classpath, NULL, NULL);
	...
	/* move reg5, $true */
	assert(0 == cesk_frame_register_load(frame, 5, CESK_STORE_ADDR_TRUE, NULL, NULL));
	...
	assert(0 == cesk_frame_register_load(frame, 31, addr, NULL, NULL));
	assert(0 == cesk_frame_register_load_from_object(frame, 5, 31, superclass, field, NULL, NULL));
	...
	assert(0 == cesk_frame_register_load(frame, 31, CESK_STORE_ADDR_NEG, NULL, NULL));
	assert(1 == cesk_store_get_refcnt(frame->store, addr));
	
	/* okay, test the garbage collector, refcount first */
	uint32_t addr4 = cesk_frame_store_new_object(frame, reloc, inst3, classpath, NULL, NULL);
	/* check the hash code*/
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	/* move reg21, addr4 */
	assert(0 == cesk_frame_register_load(frame, 21, addr4, NULL, NULL));
	/* instance-put addr2, antlr.CharScanner.literals ,reg4 */
	assert(0 == cesk_frame_store_put_field(frame, addr2, 21, superclass, field, NULL, NULL));
	/* check the hash code*/
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	...
	assert(0 == cesk_frame_register_load(frame, 20, CESK_STORE_ADDR_ZERO, NULL, NULL));
	/* check the hash code*/
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	/* addr2 is empty now */
	...
	assert(0 == cesk_frame_register_clear(frame, 21, NULL, NULL));
	...
	for(i = 0; i < frame->size; i ++)
		assert(0 == cesk_frame_register_clear(frame, i, NULL, NULL));
	assert(0 == cesk_frame_register_load(frame, 0, CESK_STORE_ADDR_NEG, NULL, NULL));
	/* check the hashcode again */
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	...
	/* create 3 objects */
	uint32_t obj1 = cesk_frame_store_new_object(frame, reloc, inst, classpath, NULL, NULL);
	uint32_t obj2 = cesk_frame_store_new_object(frame, reloc, inst2, classpath, NULL, NULL);
	uint32_t obj3 = cesk_frame_store_new_object(frame, reloc, inst3, classpath, NULL, NULL);
	...
	assert(0 == cesk_frame_register_load(frame, 1, obj1, NULL, NULL));
	assert(0 == cesk_frame_register_load(frame, 2, obj2, NULL, NULL));
	assert(0 == cesk_frame_register_load(frame, 3, obj3, NULL, NULL));
	/* verify the hashcode of the store */
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	/* build a circle */
	/* 1 --> 2 */
	assert(0 == cesk_frame_store_put_field(frame, obj1, 2, superclass, field, NULL, NULL));
	/* 2 --> 3 */                                        
	assert(0 == cesk_frame_store_put_field(frame, obj2, 3, superclass, field, NULL, NULL));
	/* 3 --> 1 */                                        
	assert(0 == cesk_frame_store_put_field(frame, obj3, 1, superclass, field, NULL, NULL));
	/* verify the hashcode of the store */
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	...
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	/* now, decref from reg */
	assert(0 == cesk_frame_register_clear(frame, 1, NULL, NULL));
	assert(0 == cesk_frame_register_clear(frame, 2, NULL, NULL));
	/* verify the hashcode of the store */
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	/* check the ref count */
	...
	assert(cesk_frame_hashcode(frame) == cesk_frame_compute_hashcode(frame));
	/* decref */
	assert(0 == cesk_frame_register_clear(frame, 3, NULL, NULL));
