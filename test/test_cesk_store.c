...
int main()
{
	...
	*cesk_object_get(object, stringpool_query("antlr/ANTLRTokdefParser"), stringpool_query("antlrTool")) = 0;
	...
	uint32_t oa1 = cesk_store_allocate(store4, ins,  CESK_OBJECT_FIELD_OFS(object, object->members[0].addrtab), 0);
	uint32_t oa2 = cesk_store_allocate(store4, ins, 12345, 0);
