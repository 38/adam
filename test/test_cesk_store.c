...
int main()
{
	...
	uint32_t    addr = cesk_store_allocate(store2, ins, 0, 0, CESK_STORE_ADDR_NULL, stringpool_query("antlr/ANTLRTokdefParser"));
	...
	*cesk_object_get(object, stringpool_query("antlr/ANTLRTokdefParser"), stringpool_query("antlrTool")) = 0;
	...
	uint32_t oa = cesk_store_allocate(store4, ins , 0, 0, CESK_STORE_ADDR_NULL, stringpool_query("antlr/ANTLRTokdefParser"));
	...
	uint32_t oa1 = cesk_store_allocate(store4, ins, CESK_STORE_ADDR_RELOC_PREFIX, CESK_OBJECT_FIELD_OFS(object, object->members[0].addrtab), CESK_STORE_ADDR_NULL,NULL);
	uint32_t oa2 = cesk_store_allocate(store4, ins, 0, 12345, CESK_STORE_ADDR_NULL, NULL);
